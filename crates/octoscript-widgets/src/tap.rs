//! `OctoscriptTap` — a tap target that does not steal the scroll.
//!
//! ## Why this exists
//!
...
script_mod! {
    ...
    // Registered into both, because a mounted Octoscript body resolves against the
    // prelude while `mod.widgets` is what other script_mod blocks extend.
    mod.widgets.OctoscriptTap = #(OctoscriptTap::register_widget(vm)){
        draw_bg +: { color: #00000000 }
    }
    mod.prelude.widgets.OctoscriptTap = mod.widgets.OctoscriptTap
}
