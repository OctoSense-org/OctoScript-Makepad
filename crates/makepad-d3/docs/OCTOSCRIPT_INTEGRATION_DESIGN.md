# Makepad-D3 × Octoscript Integration Design
...
2. [What changed in Makepad 2.0 ("Octoscript")](#2-what-changed-in-makepad-20-octoscript)
...
7. [Data binding contract (Octoscript ⇄ Rust)](#7-data-binding-contract-octoscript--rust)
8. [Running d3 inside sandboxed Octoscript apps (runsplash)](#8-running-d3-inside-sandboxed-octoscript-apps-runsplash)
...
## 2. What changed in Makepad 2.0 ("Octoscript")
...
- `live_design!{}` is **gone** (zero occurrences in `platform/` or `widgets/`). Its replacement is the **`script_mod!{}`** proc-macro (`platform/script/derive/src/lib.rs:18`), which embeds Octoscript source in the Rust file and generates `pub fn script_mod(vm: &mut ScriptVm) -> ScriptValue` for the enclosing module. `#(rust_expr)` splices a Rust expression's `ScriptValue` into the script — this is how Rust widget types enter the VM.
...
`register_widget(vm)` is generated by `derive(Widget)` (`widgets/derive_widget/src/derive_widget.rs:388`). A crate aggregates its modules in a crate-level `pub fn script_mod(vm: &mut ScriptVm)` (see `widgets/src/lib.rs:646`), and can run raw script with `script_eval!(vm, {...})` — the widgets crate uses that to compose the `mod.prelude.widgets` scope that Octoscript bodies import.

Sub-namespaces are plain script objects: the Liquid Glass kit registers `mod.widgets.glass = {}` then `mod.widgets.glass.GlassButtonBase = #(...)` (`widgets/src/glass_panel.rs:16-22`), which is why Octoscript code writes `glass.GlassButton{...}`. **This is the exact pattern `d3.*` will follow.**
...
- **`Vector` widget** (`widgets/src/vector.rs:5-38`): a declarative, SVG-like scene graph **inside Octoscript script**: `Path` (full SVG `d:` path-data strings, tweenable), `Rect/Circle/Ellipse/Line/Polyline/Polygon`, `Group`, `Gradient/RadGradient/Stop`, `Filter/DropShadow`, transform nodes (`Rotate/Scale/Translate/SkewX/SkewY`), and `Tween` for animation. This means simple d3-style graphics can be authored *entirely in Octoscript*, and our Rust shape generators can also emit SVG path data for it.

Custom shaders are still available; they are now written in Octoscript script inside `script_mod!` and bound to `#[repr(C)]` structs with `#[derive(Script, ScriptHook)]` + `#[deref] draw_super: DrawQuad` and registered via `#(DrawX::script_shader(vm))` with inheritance `..mod.draw.DrawQuad` (`draw/src/shader/draw_quad.rs:1-108`). Math pods are `Vec2f/Vec3f/Vec4f`.
...
- Full d3.js API parity in the Octoscript DSL itself (the DSL is not JavaScript; the grammar surface is charts + options + data + events).
- Upstream makepad changes as a hard dependency. An upstream extension hook is proposed ([§8.3](#83-upstream-proposal-octoscript-vm-module-hook)) but every goal above is achievable without it.
...
```
┌────────────────────────────────────────────────────────────────────┐
│ Octoscript apps (3 host tiers)                                         │
│  A) Rust app script_mod! blocks   B) Octoscript widget bodies          │
│     d3.LineChart{...}                (runsplash / AI chat)         │
│                                   C) .octoscript files (Studio)        │
├────────────────────────────────────────────────────────────────────┤
│ mod.d3  (script namespace)                 ← src/octoscript/mod.rs     │
│   d3.BarChart d3.LineChart d3.AreaChart d3.PieChart d3.Treemap …  │
│   d3.theme (color schemes)   D3Octoscript (sandbox host widget)        │
├────────────────────────────────────────────────────────────────────┤
│ Widget layer (NEW: src/octoscript/)                                    │
...
3. **Octoscript is the user interface; Rust is the engine.** Everything a Octoscript author can configure is either a `#[live]` property (declarative) or a `script_call` method (imperative). Everything interactive fires a `ScriptFnRef` callback.
...
Crate-level entry point (new `src/octoscript/mod.rs`, re-exported from `lib.rs`):
...
script_mod! {
    ...
    mod.d3.BarChart = set_type_default() do mod.d3.BarChartBase{
        width: Fill
        height: 300              // sandbox-friendly: Fill collapses in Fit parents (octoscript.md rule)
...
- **Namespace `d3`, not additions to `mod.widgets`** — avoids collision with the built-in `LineChart`/`BarChart` and gives Octoscript authors an unambiguous `d3.` prefix (mirrors `glass.`).
- **Defaults tuned for Octoscript bodies**: fixed default height (Octoscript `Fit`-parent rule), transparent background (glass backdrops), theme colors overridable per-instance.
...
| Method (Octoscript side) | Args | Semantics |
|---|---|---|
...
so Octoscript authors write `d3.BarChart{ on_click: |i| ui.status.set_text("bar " + i) }`.
...
## 7. Data binding contract (Octoscript ⇄ Rust)
...
Live-update loop, the killer demo for Octoscript apps (net-enabled sandbox):
...
Octoscript-authoring constraints we inherit (from `octoscript.md`) and bake into defaults: containers need `height: Fit`/fixed heights, `:=` ids for anything addressed via `ui.`, `for` loops render build-time only (dynamic chart lists go through `on_render` + `ui.x.render()`), numbers stay numeric (`"" + n` to display).

---

## 8. Running d3 inside sandboxed Octoscript apps (runsplash)
...
### 8.3 Upstream proposal: octoscript VM module hook
...
AI Chat's `runsplash` generation is prompt-driven (`octoscript.md` is the DSL guide). Once a host embeds `D3Octoscript`, add a **`d3-octoscript.md` guide** (this repo, `docs/`) in the same format — widget list, data shapes, gotchas — so agents can be pointed at it to generate d3 charts inside octoscript apps. Deliverable of Phase 4.
...
2. The `live_design!` shader block → `script_mod!` entries: `mod.d3.DrawMesh3d = mod.std.set_type_default() do #(DrawMesh3d::script_shader(vm)){ ..mod.draw.DrawQuad  vertex: fn(){...} pixel: fn(){...} }`. Shader fns are Octoscript script; `mod.shader` provides `Sdf2d`, math comes from `mod.math`.
...
|---|---|
| `live_design!{ ... }` | `script_mod!{ ... }` (Octoscript syntax: no commas needed, `:=` ids, `+:` merge) |
...
- Create `octoscript-2.0` migration branch; bump dep there to `branch = "dev"` @ latest (or rev-pin `4f9ce7a8b`).
...
**Phase 2 — `src/octoscript/` widget layer MVP (1 week)**
...
- New example `examples/octoscript_demo`: 2.0-style app (`AppMain::script_mod`) whose UI is written in Octoscript and drives charts from script (button → `set_data`, slider → domain).

**Phase 3 — Sandbox host (3 days)**
- `D3Octoscript` widget ([§8.2](#82-solution-d3octoscript-host-widget-tier-b)); demo: markdown view rendering a ```` ```runsplash ```` block with a `d3.*` chart via template override.
- Draft upstream hook PR ([§8.3](#83-upstream-proposal-octoscript-vm-module-hook)).
...
- Each wave: register in `mod.d3`, add a octoscript usage snippet to `docs/d3-octoscript.md`.

**Phase 5 — Docs & release (2 days)**
- `docs/d3-octoscript.md` authoring guide (octoscript.md-style, for humans and AI generation).
- README update (2.0 requirement, octoscript quick start), CHANGELOG, tag `0.2.0`, fold `main` ← `octoscript-2.0`.
...
| `script_call` arg conventions could evolve | method layer churn | isolate all heap/arg parsing in one `src/octoscript/vm_data.rs` helper module (single choke point) |
...
| Sandbox instruction budget (200k) with big datasets | truncated eval in runsplash | data via `set_data`/`net` (runtime calls are budgeted per-event, not per-eval); document dataset guidance in `d3-octoscript.md` |
...
| Upstream hook rejected | Tier B only | `D3Octoscript` is fully self-sufficient; revisit if AI Chat exposes its own extension point |
...
tests pass, and `examples/octoscript_demo` runs with zero script errors — all
five `d3.*` charts render from Octoscript DSL, and a `d3.Octoscript` sandbox
...
| `d3.*` namespace + prelude injection | `src/octoscript/mod.rs` (`mod.d3 = {}`), `src/octoscript/charts.rs` (`mod.prelude.widgets.d3 = mod.d3`) |
| Chart widgets (Bar, Line, Area, Scatter, Pie) | `src/octoscript/charts.rs` — `DrawVector` rendering, d3 scales/ticks, `data:`/`labels:` apply-time parsing, `set_data`/`set_labels`/`set_domain`/`data()` script calls, `on_click`/`on_hover` `ScriptFnRef` events, demo data when empty |
| VM data conversion choke point | `src/octoscript/vm_data.rs` |
| Sandbox host (Tier B) | `src/octoscript/host.rs` — `d3.Octoscript`, isolate VM + `crate::script_mod` registration + body eval |
| Registration entry point | `makepad_d3::script_mod(vm)` in `src/lib.rs` |
| Proof app | `examples/octoscript_demo` (`cargo run --example octoscript_demo`) |
...
Phase 5 (authoring guide `docs/d3-octoscript.md`, release).
...
`octoscript_demo` gallery, zero script errors, clippy-clean, 813 tests):

| Wave | Widgets | Module |
|---|---|---|
| MVP | BarChart, LineChart, AreaChart, ScatterChart, PieChart | `src/octoscript/charts.rs` |
| 1 — statistical | Histogram, Heatmap, RadarChart, BoxPlot | `src/octoscript/charts_stat.rs` |
| 2 — hierarchies | Treemap, Sunburst, CirclePack, TreeChart | `src/octoscript/charts_hier.rs` |
| 2 — flows | Sankey, ChordDiagram, ArcDiagram | `src/octoscript/charts_flow.rs` |
| 3 — networks/density/geo | ForceGraph, Hexbin, Ridgeline, Horizon, Contour, Globe | `src/octoscript/charts_net.rs` |
| 4 — 3D | Surface3D, Scatter3D, Bar3D | `src/octoscript/charts_3d.rs` |

Phase 5 deliverables: `docs/d3-octoscript.md` (Octoscript authoring guide for all
