# d3.* Octoscript DSL Guide

How to use makepad-d3's chart widgets from Octoscript DSL — written in the same
style as makepad's `octoscript.md` so it can be handed to humans or AI agents
...
| Widget | Purpose |
|---|---|
| `d3.Octoscript` | Evaluates a Octoscript body string in an **isolated VM with `d3.*` registered** — the runsplash path. Feed with `set_text` (Rust: `D3OctoscriptRef::set_text` / script: `ui.host.set_text("...")`). No networking; body-level helper `fn`s have no `ui` global (inline handlers do). |
