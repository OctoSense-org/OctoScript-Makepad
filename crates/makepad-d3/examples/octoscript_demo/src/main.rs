//! makepad-d3 × Octoscript gallery
//!
//! The whole UI below is Octoscript DSL. Every `d3.*` chart widget in the
...
script_mod! {
    ...
    // `[..] [..]` parses as indexing in Octoscript.
    let bar_data = [
    ...
    startup() do #(App::script_component(vm)){
        ui: Root{
            main_window := Window{
                ...
                window.title: "makepad-d3 × Octoscript — full d3.* gallery"
                body +: {
                    padding: 12
                    flow: Down
                    spacing: 8
                    ...
                    View{ width: Fill height: Fit flow: Right spacing: 12 align: Align{y: 0.5}
                        Label{
                            text: "makepad-d3: 21 chart widgets in Octoscript DSL"
                            draw_text.color: #fff
                    ...
                    // Row 6: sandboxed octoscript app (isolated VM, runsplash-style)
                    Label{
                        text: "Sandboxed octoscript app (d3.Octoscript host — an isolated VM with d3.* registered):"
...
/// Body for the sandboxed `d3.Octoscript` host — evaluated in an isolated VM,
