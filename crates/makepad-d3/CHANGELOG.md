...
### Makepad 2.0 "Octoscript" migration

- Target **Makepad 2.0** (Script/Octoscript runtime; the Live system /
...
### The `d3.*` Octoscript namespace (new)

`makepad_d3::script_mod(vm)` registers **21 chart widgets** usable directly
from Octoscript DSL, with declarative `data:` props, `set_data(...)`-style
...
- Sandbox host: `d3.Octoscript` — evaluates runtime Octoscript bodies in an
  isolated VM with `d3.*` registered (runsplash-style)

### Docs & examples

- `examples/octoscript_demo`: full-gallery app, entire UI in Octoscript DSL.
- `docs/OCTOSCRIPT_INTEGRATION_DESIGN.md`: architecture + implementation record.
- `docs/d3-octoscript.md`: Octoscript authoring guide for all widgets (for humans
  and AI generation).
