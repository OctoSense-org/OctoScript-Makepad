...
script_mod! {
    ...
    mod.d3.OctoscriptBase = #(D3Octoscript::register_widget(vm))

    mod.d3.Octoscript = set_type_default() do mod.d3.OctoscriptBase{
        width: Fill height: Fit
    }
}

const OCTOSCRIPT_PREFIX: &str = "use mod.prelude.widgets.*\nView{height:Fit, ";
const OCTOSCRIPT_EVAL_INSTRUCTION_LIMIT: usize = 200_000;

/// Sandboxed Octoscript-body host with `d3.*` registered (see module docs).
#[derive(Script, ScriptHook, WidgetRef, WidgetRegister)]
pub struct D3Octoscript {
    #[uid]
    uid: WidgetUid,
    #[source]
...
impl D3Octoscript {
    ...
    fn eval_body(&mut self, cx: &mut Cx) {
        ...
        let code = format!("{}{}", OCTOSCRIPT_PREFIX, body);
        ...
        let new_view = cx.with_script_vm_id(vm_id, |vm| {
            let value = vm.with_instruction_limit(OCTOSCRIPT_EVAL_INSTRUCTION_LIMIT, |vm| {
...
impl WidgetNode for D3Octoscript {
