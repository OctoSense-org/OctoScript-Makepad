//! Chart widgets registered under the `d3.*` Octoscript namespace.
//!
...
//! `d3.BarChart{}` in a Octoscript body shows something immediately.
...
script_mod! {
    ...
    // Octoscript bodies open with `use mod.prelude.widgets.*`, so this line is
...
/// A categorical bar chart, exposed to Octoscript as `d3.BarChart`.
...
/// An x/y line chart, exposed to Octoscript as `d3.LineChart`.
#[derive(Script, Widget)]
...
/// An x/y area chart, exposed to Octoscript as `d3.AreaChart`.
#[derive(Script, Widget)]
...
/// An x/y scatter plot, exposed to Octoscript as `d3.ScatterChart`.
#[derive(Script, Widget)]
...
/// A pie/donut chart, exposed to Octoscript as `d3.PieChart`.
///
