# Makepad 2.0 "Octoscript" Update — Design & Port Plan

## Goal

Update makepad-plot from Makepad 1.0 (`live_design!` DSL, wyeworks rev `53b2e5c84`)
to **Makepad 2.0** and make **every plot widget usable in Octoscript fashion** — i.e.
declarable and scriptable from the Octoscript DSL (`script_mod!` / ScriptVm), exactly like
the built-in Makepad 2.0 widgets (`Button`, `Label`, `CandlestickChart`, …).

## What "Octoscript fashion" means (from makepad dev branch)

Makepad 2.0 replaces `live_design!` with the **Octoscript script system**:
...
- Octoscript scripts then declare widgets declaratively (`line := LinePlot{ title: "..." }`)
...
- **Default demo data**: when a chart has no data it self-populates with generated
  demo data, so a bare `LinePlot{}` in a Octoscript script renders immediately
  (same convention as makepad's `chart.rs`).
- **Octoscript-callable methods** via `script_call` (dispatched from
...
and in Octoscript: `use mod.plot.*`.

### Demo (`examples/plot_demo.rs`)

Octoscript-declared demo app (the `plot_demo` bin): a window with a scrollable grid
showing **all ported chart widgets**, each rendering its built-in demo data, with
pan/zoom on cartesian charts — the whole UI written in the Octoscript DSL.
...
| widget `Ref` rust setter APIs     | kept, **plus** Octoscript `script_call` methods |
...
- **Octoscript array literals are typed arrays, not objects**: parse method args with
...
- The old 5.6k-line Rust demo (`plot_demo.rs`) is replaced by the Octoscript demo.
