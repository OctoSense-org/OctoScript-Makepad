...
pub fn inspectable(tree: &mut splash_render::UiNode) -> Vec<serde_json::Value> {
    fn visit(
        n: &mut splash_render::UiNode,
        path: String,
...
pub fn prepare(card: &str, data: &serde_json::Value, dir: &Path) -> Result<PreparedCard, String> {
    ...
    let mood = splash_ui_l0::card_theme(card).unwrap_or_else(|| "dark".into());
    if splash_ui_l0::kit_pack::contains(root) {
        if !splash_ui_l0::card_theme_axes(card).is_empty() {
    ...
    let mut parts = vec![read("_palette_dark.splash")?];
    if mood != "dark" {
        parts.push(read(&format!("_palette_{mood}.splash"))?);
    }
    let mut axes = splash_ui_l0::card_theme_axes(card);
    let order = [
    ...
    for (axis, value) in axes {
        ...
        let file = if axis == "accent" {
            format!("_axis_accent_{value}_{mood}.splash")
        } else {
            format!("_axis_{axis}_{value}.splash")
        };
        parts.push(read(&file)?);
    }
    for name in ["_derive_color.splash", "_derive.splash"] {
    ...
    if pack_path.is_file() {
        ...
        parts.push(splash_ui_l0::kit_pack::theme_source(&pack));
    }
    parts.push(read("_kit.splash")?);
    parts.push(splash_ui_l0::kit::lower(&root));
    ...
    let tree = splash_render::build(&source, |_| {})
...
pub fn texts(node: &splash_render::UiNode) -> Vec<String> {
    let mut out = Vec::new();
...
pub fn images(node: &splash_render::UiNode) -> Vec<String> {
    let mut out = Vec::new();
    fn walk(n: &splash_render::UiNode, out: &mut Vec<String>) {
        if matches!(n.kind,splash_render::NodeKind::Image | splash_render::NodeKind::Svg) {
