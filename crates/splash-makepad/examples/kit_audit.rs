...
fn source_tree(
    node: &mut splash_render::UiNode,
    selectors: &mut std::collections::BTreeMap<String, bool>,
) -> Result<(), String> {
    if let Some(config) = splash_makepad::design::kit_contract(node)? {
    ...
    for mut child in std::mem::take(&mut node.children) {
        ...
        if radio || button {
            ...
            let expected = splash_makepad::design::prepare(&source)?;
            if splash_makepad::design::to_makepad_ui(&child)?
                != splash_makepad::design::to_makepad_ui(&expected)?
            {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for path in paths {
        ...
        let result = (|| -> Result<usize, String> {
            ...
            .map_err(|e| e.to_string())?;
            let prepared = splash_makepad::l0::prepare(&card, &data, Path::new(&args[3]))?;
            if !prepared.native_components {
                return Err("did not use registered components".into());
            }
            let before =
                std::fs::read_to_string(Path::new(&args[2]).join(format!("{name}.splash")))
                    .map_err(|e| e.to_string())?;
            let before = splash_makepad::design::prepare(&before)?;
            let expected = splash_makepad::design::to_makepad_ui(&before)?;
            let mut preserved = prepared.tree.clone();
            source_tree(&mut preserved, &mut Default::default())?;
            let actual = splash_makepad::design::to_makepad_ui(&preserved)?;
