...
//!
//! The Octoscript DSL evaluates (in the makepad-script VM) to a tree of plain data
...
pub enum NodeKind {
    ...
    /// `octoscript-widgets`. A DSL node cannot carry shader source — MPSL compiles
    /// at build time — but it can select a shader that was compiled.
    Shader,
    Sdf,
    // ---- Material components -------------------------------------------
    // The reference catalog (Octoscript-Android) states components *semantically*
    ...
    /// space and puts a real WebView there — the hybrid Octoscript-OH's own cards
...
pub struct Attrs {
    ...
    /// on a text node. The reference catalog (Octoscript-Android, rendered with real
