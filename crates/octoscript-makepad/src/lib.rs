//! makepad render backend for Octoscript.
//!
//! The shared [`octoscript_render`] core evaluates the Octoscript DSL (in the
...
//! layout, and theming, and keeps this backend small. Octoscript-OH's ArkUI backend
...
fn widget_name(kind: NodeKind) -> &'static str {
    match kind {
        ...
        // octoscript-widgets' shape-morph blob — it renders nothing on an isolate and
        ...
        // NAME from the DSL — the route `octoscript-widgets` already documents, and
...
fn emit_click_overlay(node: &UiNode, out: &mut String, depth: usize, resolved: bool) {
    ...
    //
    // `OctoscriptTap` in octoscript-widgets is the real answer and cannot be reached: a
    // widget this workspace defines does not resolve inside the isolate VM a
    // mounted Octoscript allocates. See that module.
    // No edge strip, and no need for one. The target below is `OctoscriptTap`,
    ...
    // parse.
    //
    // `OctoscriptTap` — reachable now. A Octoscript isolate takes makepad's own script
...
/// `0xAARRGGBB` (the Octoscript colour word) → makepad `#RRGGBBAA`.
...
mod tests {
    ...
    fn a_tappable_container_gets_a_non_capturing_target_over_it() {
        ...
        // `OctoscriptTap`, not a Button. Any Button captures the finger on
