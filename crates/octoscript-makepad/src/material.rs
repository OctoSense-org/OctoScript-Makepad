...
//!
//! The reference catalog (Octoscript-Android, rendered with real
...
/// A section header, as `screens/kit.octoscript` spells one: the only Text node in
...
fn sl_native(node: &UiNode, frac: f32) -> UiNode {
    ...
    // ask: a widget inside a mounted Octoscript body is not reachable by id even
