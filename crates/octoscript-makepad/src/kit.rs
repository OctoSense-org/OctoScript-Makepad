//! Assemble a directory of `.octoscript` files into one script.
//!
//! The Octoscript DSL has no `import`. A kit that spans more than one file is
...
//! | file | position | holds |
//! |---|---|---|
//! | `_kit.octoscript` | first | tokens and helpers every screen uses |
//! | everything else | sorted, between | one file per screen group, defining `fn screen_*` |
//! | `_index.octoscript` | last | the index screen and the route dispatch |
//!
...
const HEAD: &str = "_kit.octoscript";
const TAIL: &str = "_index.octoscript";

/// Concatenate every `.octoscript` file in `dir` into one script, in the order
...
///
/// Fails if `dir` cannot be read or if either `_kit.octoscript` or `_index.octoscript`
...
pub fn concat_kit(dir: &Path) -> io::Result<String> {
    ...
    for entry in std::fs::read_dir(dir)? {
        let path = entry?.path();
        if path.extension().and_then(|e| e.to_str()) != Some("octoscript") {
            continue;
        }
...
///
/// `platform_channels` and `pedometer` call `invoke(tool)`. On Octoscript-OH the
...
pub fn register_stub_capabilities(vm: &mut octoscript_render::makepad_script::ScriptVm) {
    use octoscript_render::makepad_script::makepad_live_id::*;
    let f = octoscript_render::add_global_fn(
        vm,
        &[(id!(tool), octoscript_render::makepad_script::ScriptValue::NIL)],
        |vm, a| {
            let tool = octoscript_render::string_prop(vm, a, id!(tool)).unwrap_or_default();
    ...
    let g = octoscript_render::add_global_fn(
        vm,
        &[
            (id!(key), octoscript_render::makepad_script::ScriptValue::NIL),
            (id!(dflt), octoscript_render::makepad_script::ScriptValue::NIL),
        ],
        |vm, a| {
            let key = octoscript_render::string_prop(vm, a, id!(key)).unwrap_or_default();
            let dflt = octoscript_render::num_prop(vm, a, id!(dflt)).unwrap_or(0.0);
            octoscript_render::makepad_script::ScriptValue::from_f64(
                octoscript_render::state::get_or_seed(&key, dflt),
            )
        },
    );
