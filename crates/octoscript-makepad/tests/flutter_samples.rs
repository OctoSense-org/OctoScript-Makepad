...
//! below therefore asserts on a string only that screen emits. Octoscript-OH learned
//! this the hard way: two of its 28 catalog screens were reachable and wrong.

use octoscript_makepad::{kit, to_makepad_ui};
...
fn render(kit: &str, route: &str) -> String {
    ...
    let tree = octoscript_render::build(&src, kit::register_stub_capabilities)
...
fn cases() -> Vec<(String, &'static str)> {
    ...
    for (route, marker) in [
        ...
        ("asset_transformation", "octoscript:// resolves a request to bytes"),
...
fn every_indexed_sample_is_reachable() {
    ...
    let dirs = std::fs::read_dir(kit_dir())
        ...
        .filter(|n| n.ends_with(".octoscript") && !n.starts_with('_'))
        .map(|n| n.trim_end_matches(".octoscript").to_string())
        .collect::<Vec<_>>();

    assert_eq!(dirs.len(), 27, "one .octoscript per flutter/samples directory");
...
fn helper_calls_have_the_declared_arity() {
    ...
    // occurrence truncated `m_section("octoscript:// resolves ...")` mid-call and
...
fn kit_assembles_head_first_and_index_last() {
    ...
    // markers that name a .octoscript file.
    let markers: Vec<&str> = kit
        .lines()
        .filter_map(|l| l.strip_prefix("// ---- "))
        .filter(|l| l.ends_with(".octoscript"))
        .collect();

    assert_eq!(markers.first(), Some(&"_kit.octoscript"), "_kit.octoscript is first");
    assert_eq!(markers.last(), Some(&"_index.octoscript"), "_index.octoscript is last");
    // 27 sample directories, plus the head and the tail.
    assert_eq!(markers.len(), 29, "one marker per .octoscript file");
...
fn a_control_changes_what_the_screen_renders() {
    ...
    for (route, action, marker) in cases {
        ...
        assert!(
            octoscript_render::state::apply(action),
...
fn probe2() {
    ...
    octoscript_render::state::apply("set:m3_switch=!");
    eprintln!("PROBE store now = {}", octoscript_render::state::get("m3_switch", 1.0));
...
fn a_control_behind_another_control_still_works() {
    ...
    for (route, setup, action, before_frag, after_frag) in cases {
        octoscript_render::state::reset();
        assert!(octoscript_render::state::apply(setup), "{setup:?} is not an action");
        let before = render(&kit, route);
        if !before.contains(before_frag) {
            failures.push(format!("{route}: {before_frag:?} missing after {setup:?}"));
        }
        assert!(octoscript_render::state::apply(action), "{action:?} is not an action");
        let after = render(&kit, route);
        if !after.contains(after_frag) {
...
fn a_capability_reread_is_inert_on_this_backend() {
    ...
    assert!(octoscript_render::state::apply("set:pvs_reads=+1"));
