...
The workspace uses the checked-out sibling `../makepad`: `octoscript-render` takes
...
`ScriptVm::eval_checked` API. The L0 integration tests use sibling `../octoscript`.

## Build

### L0 design-kit validation

The lab's `octoscript-beauty-host` Studio runnable mounts L0 cards through
...
With the `makepad`, `octoscript` and `octoscript-makepad` checkouts present:

```sh
cargo test --release -p octoscript-render -p octoscript-makepad
```
